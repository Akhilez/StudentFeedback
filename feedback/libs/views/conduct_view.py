...
from feedback.libs.view_helper import feedback_running, invalid_user_page, get_cur_time_offset, get_main_of, \
    get_subordinate_of
from feedback.models import Session, SubordinateSession, Initiation, Attendance, Student, Classes
...
def get_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'makemain' in request.POST:
            return make_main_result(request.POST['makemain'])
...
def make_main_result(session):
    SubordinateSession.objects.create(main=Session.objects.get(session_id=session))
    return redirect('/')
...
def add_my_sessions(user, context):
    ...
    for session in all_sessions:
        ...
        if session.timestamp.date() == datetime.date.today() and get_session_length() > cur_time:
            ...
            try:
                SubordinateSession.objects.get(main=session)
            except:
                running_mains.append(session.session_id)

    context['allSessions'] = all_sessions
    context['running'] = running_sessions
    context['running_mains'] = running_mains


def is_main(session):
    for sess in SubordinateSession.objects.all():
        if sess.main == session and sess.subordinate is None:
            return True
    return False
...
def confirm_session_result(request):
    # is the session required to split? if yes, the current session is main
    main = True if len(request.POST.getlist('main')) > 0 else False
    ...
    if main:
        SubordinateSession.objects.create(main=session)
    else:
        # check if session exists
        for session0 in Session.objects.all().order_by('-timestamp'):
            if init_obj == session0.initiation_id and session != session0:
                main_session = SubordinateSession.objects.get(main=session0)
                main_session.subordinate = session
                main_session.save()
                break
...
def take_attendance_result(request, template, context):
    ...
    2. if session is subordinate session, add absent students to context
    ...
    if session is not None and session.main:
        context['main'] = True
...
def get_absent_students(session):
    ...
    3. filter attendance from main session
    3.1 filter attendance from subordinate session
    ...
    # 3. filter attendance from main session
    main = get_main_of(session)
    if main is not None:
        filter_attendance(main, students)

    # 3.1 filter attendance from subordinate session
    subordinate = get_subordinate_of(session)
    if subordinate is not None:
        filter_attendance(subordinate, students)
